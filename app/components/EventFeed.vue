...
function statusWord(e: DiscoEvent) {
  return e.status === 'success' ? 'passed' : e.status === 'failed' ? 'failed' : 'canceled'
}
...
<template>
  ...
  >
    <li
      v-for="e in events"
      :key="`${e.demo ? 'd' : 's'}${e.id}`"
      class="grid grid-cols-[4rem_5.5rem_1fr_auto] items-baseline gap-x-4 border-b border-night-900 py-2.5 text-lg"
    >
